...
import 'package:drawerbehavior/controllers/hidden_drawer_controller.dart';
import 'package:drawerbehavior/hidden_drawer/hidden_drawer_bloc.dart';
import 'package:drawerbehavior/hidden_drawer/screen_hidden_drawer.dart';
import 'package:drawerbehavior/menu/hidden_menu.dart';
import 'package:drawerbehavior/menu/item_hidden_menu.dart';
import 'package:drawerbehavior/provider/HiddenDrawerBloc.dart';
import 'package:drawerbehavior/provider/HiddenDrawerProvider.dart';

class HiddenDrawerMenu extends StatefulWidget {
  /// List item components.drawer.menu and respective screens
  final List<ScreenHiddenDrawer> screens;

  /// position initial item selected in components.drawer.menu( sart in 0)
  ...
  /// enable auto title in appbar with components.drawer.menu item name
    ...
    return Stack(
      children: [
        ...
        GestureDetector(
          ...
          onTap: () {
            if (_controller.state == MenuState.open) _bloc.toggle();
          },
        )
      ],
    );
  ...
  animateContent(Widget content) {
    return StreamBuilder(
        ...
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          ...
          final contentScale = 1 - ((1-widget.scale) * animatePercent);
          var contentPerspective = 0.0;
          final cornerRadius = widget.borderRadius * animatePercent;
